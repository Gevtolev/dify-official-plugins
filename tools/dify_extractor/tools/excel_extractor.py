...
class ExcelExtractor(BaseExtractor):
    ...
    def __init__(self, file_bytes: bytes, file_name:str, encoding: Optional[str] = None, autodetect_encoding: bool = False):
        ...
        if file_extension == ".xlsx":
            ...
            for sheet_name in wb.sheetnames:
                ...
                for index, row in df.iterrows():
                    md_content += "| " + " | ".join([str(cell) if pd.notna(cell) else "" for cell in row]) + " |\n"
                    ...
                    documents.append(
                        Document(page_content=";".join(page_content), metadata={"source": self._file_name})
                    )
