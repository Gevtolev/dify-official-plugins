...
class CSVExtractor(BaseExtractor):
    ...
    def extract(self) -> ExtractorResult:
        ...
        md_content = ""
        try:
            csvfile = TextIOWrapper(BytesIO(self._file_bytes), encoding=self._encoding, newline="")
            md_content, docs = self._read_from_file(csvfile)
        except UnicodeDecodeError as e:
            if self._autodetect_encoding:
                detected_encodings = detect_file_encodings(self._file_bytes)
                for encoding in detected_encodings:
                    try:
                        csvfile = TextIOWrapper(BytesIO(self._file_bytes), encoding=encoding.encoding, newline="")
                        md_content, docs = self._read_from_file(csvfile)
                        break
    ...
    def _read_from_file(self, csvfile) -> tuple[str, list[Document]]:
        ...
        try:
            ...
            for i, row in df.iterrows():
                ...
                # markdown row
                markdown_row = "| " + " | ".join(str(row[col]).strip() for col in df.columns) + " |\n"
                markdown_content += markdown_row

        except csv.Error as e:
            raise e

        return ExtractorResult(md_content=markdown_content, documents=docs)