...
class PartitionTool(Tool):
    ...
    def validate_api_url(self) -> None:
        ...
        try:
            ...
            raise ToolProviderCredentialValidationError(f"validate api_url failed. reason:{e}")
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        req = operations.PartitionRequest(
            partition_parameters=shared.PartitionParameters(
                ...
                languages=json.loads(tool_parameters.get("languages", "[]") if tool_parameters.get("languages") else "[]"),
                ...
                **{k: v for k, v in json.loads(tool_parameters.get("advanced_options", "{}")).items()
                   if k in [
                       'coordinates', 'content_type', 'encoding',
                       'extract_image_block_types', 'gz_uncompressed_content_type',
                       'include_page_breaks', 'ocr_languages',
                       'output_format', 'pdf_infer_table_structure', 'skip_infer_table_types',
                       'starting_page_number', 'unique_element_ids', 'xml_keep_tags',
                       'combine_under_n_chars', 'include_orig_elements',
                       'multipage_sections', 'new_after_n_chars',
                       'overlap', 'overlap_all', 'similarity_threshold'
                   ] and v is not None}
            ),
        )
        try:
            ...
            for element in elements:
                ...
                if "orig_elements" in element["metadata"]:
                    ...
                    orig_elements = json.loads(self.extract_orig_elements(element["metadata"]["orig_elements"]))
                    ...
                    for orig_element in orig_elements:
                        if orig_element["type"] == "Image":
                            if orig_element["metadata"].get("image_base64"):
                                ...
                                extension = guess_extension(mime_type)
                                image_name = f"image_{orig_element['element_id']}{extension}"
                                file_res = self.session.file.upload(
                                ...
                                images.append(file_res)
                                orig_element["metadata"]["preview_url"] = file_res.preview_url
